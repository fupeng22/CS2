$(function () {
    ...
    var UpdateConfirmCheckStatusURL = "/Customer_SceneConfirmCheckDetail/UpdateConfirmCheckStatus?swbdID_Local=";
    var BakUpdateConfirmCheckStatusURL = "/Customer_SceneConfirmCheckDetail/BakUpdateConfirmCheckStatus?swbdIDs=";

    var QueryURL = "/Customer_SceneConfirmCheckDetail/GetData?Detail_wbID=" + encodeURI($("#hid_wbID").val()) + "&Detail_swbSerialNum=" + encodeURI($("#txtSubWayBillCode").val());
    ...
    $("#btnQuery_Detail").click(function () {
        QueryURL = "/Customer_SceneConfirmCheckDetail/GetData?Detail_wbID=" + encodeURI($("#hid_wbID").val()) + "&Detail_swbSerialNum=" + encodeURI($("#txtSubWayBillCode").val());
    ...
    $("#btnConfirmScenseCheck_SubWayBill").click(function () {
        ...
        if (selects.length != 0) {
            ...
            BakUpdateConfirmCheckStatus(ids.join(","), 1);
        } else {
            reWriteMessagerAlert('操作提示', "请选择需要审核确认的物品信息", 'error');
            return false;
        }
    });

    $("#btnUnConfirmScenseCheck_SubWayBill").click(function () {
        var selects = _$_datagrid.treegrid("getSelections");
        var ids = [];
        if (selects.length != 0) {
            for (var i = 0; i < selects.length; i++) {
            ...
            BakUpdateConfirmCheckStatus(ids.join(","), 0);
        } else {
            reWriteMessagerAlert('操作提示', "请选择需要取消审核的物品信息", 'error');
            return false;
        }
    });
    ...
    function BakUpdateConfirmCheckStatus(swbdIDs, ConfirmCheckStatus) {
        var swbdIDs_Local = encodeURI(swbdIDs);
        var ConfirmCheckStatus_Local = encodeURI(ConfirmCheckStatus);

        var bOK = false;
        $.ajax({
            type: "POST",
            url: BakUpdateConfirmCheckStatusURL + swbdIDs_Local + "&ConfirmCheckStatus_Local=" + ConfirmCheckStatus_Local,
            data: "",
            async: false,
            cache: false,
            beforeSend: function (XMLHttpRequest) {
    ...
    function SeleAll() {
        ...
        for (var i = 0; i < roots.length; i++) {
            ...
            for (var j = 0; j < childrens.length; j++) {
                _$_datagrid.treegrid("select", childrens[j].ID)
            }
        }
    }
        ...
        for (var i = 0; i < roots.length; i++) {
            ...
            for (var j = 0; j < childrens.length; j++) {
                ...
                for (var k = 0; k < selects.length; k++) {
                    if (childrens[j].ID == selects[k].ID) {
                        bSele = true;
                    }
                }
                if (bSele) {
                    _$_datagrid.treegrid("unselect", childrens[j].ID)
                } else {
                    _$_datagrid.treegrid("select", childrens[j].ID)
                }
            }
