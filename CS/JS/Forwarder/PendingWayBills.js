$(function () {
    ...
    var HanldePendingSubWayBillURL = "/Forwarder_PendingWayBillsDetail/Index?Detail_wbSerialNum=";
    ...
    //var QueryURL = "/Forwarder_PendingWayBills/GetData?txtSubWayBillCode=" + encodeURI($("#txtSubWayBillCode").val()) + "&txtCode=" + encodeURI($("#txtCode").val()) + "&inputBeginDate=" + encodeURI($("#inputBeginDate").val()) + "&inputEndDate=" + encodeURI($("#inputEndDate").val()) + "&txtGCode=" + encodeURI($("#txtGCode").val()) + "&hidSearchType=" + encodeURI($("#hidSearchType").val()) + "&txtVoyage=" + encodeURI(_$_ddCompany.combobox("getValue")) + "&txtGSubWayBillCode=" + encodeURI($("#txtGSubWayBillCode").val());
    var QueryURL = "/Forwarder_PendingWayBills/GetData?txtCode=" + encodeURI($("#txtCode").val()) + "&inputBeginDate=" + encodeURI($("#inputBeginDate").val()) + "&inputEndDate=" + encodeURI($("#inputEndDate").val()) + "&txtGCode=" + encodeURI($("#txtGCode").val()) + "&hidSearchType=" + encodeURI($("#hidSearchType").val()) + "&txtVoyage=" + encodeURI(_$_ddCompany.combobox("getValue"));
    ...
    $("#btnQuery").click(function () {
        if ($("#txtCode").val() == "") {
            ...
            $("#hidSearchType").val(1);
            QueryURL = "/Forwarder_PendingWayBills/GetData?txtCode=" + encodeURI($("#txtCode").val()) + "&inputBeginDate=" + encodeURI($("#inputBeginDate").val()) + "&inputEndDate=" + encodeURI($("#inputEndDate").val()) + "&txtGCode=" + encodeURI($("#txtGCode").val()) + "&hidSearchType=" + encodeURI($("#hidSearchType").val()) + "&txtVoyage=" + encodeURI(_$_ddCompany.combobox("getValue"));
    ...
    $("#btnExcel").click(function () {
        ...
        PrintURL = "/Forwarder_PendingWayBills/Excel?txtCode=" + encodeURI($("#txtCode").val()) + "&inputBeginDate=" + encodeURI($("#inputBeginDate").val()) + "&inputEndDate=" + encodeURI($("#inputEndDate").val()) + "&txtGCode=" + encodeURI($("#txtGCode").val()) + "&hidSearchType=" + encodeURI($("#hidSearchType").val()) + "&txtVoyage=" + encodeURI(_$_ddCompany.combobox("getValue")) + "&order=" + _$_datagrid.datagrid("options").sortOrder + "&sort=" + _$_datagrid.datagrid("options").sortName + "&page=1&rows=10000000&browserType=" + browserType;
        if (_$_datagrid.datagrid("getData").rows.length > 0) {
    ...
    _$_datagrid.datagrid({
        ...
        columns: [[
                    ...
                    { field: 'PendingSubWayBillCount', title: '待处理分运单数', width: 100, sortable: true,
                        sorter: function (a, b) {
                            return (a > b ? 1 : -1);
                        },
                        formatter: function (value, rowData, rowIndex) {
                            var sRet = "";
                            if (rowData.PendingSubWayBillCount != "0") {
                                sRet = rowData.PendingSubWayBillCount + "  " + "<input type='button' class='clsHandlePendingSubWayBills' wbID='" + rowData.wbID + "' wbSerialNum='" + rowData.wbSerialNum + "' value='处理'/>";
                            } else {
                                sRet = rowData.PendingSubWayBillCount;
                            }
                            return sRet;
                        }
                    }
        ...
        onLoadSuccess: function (data) {
            ...
            var allHandlePendingSubWayBillsBtn = $(".clsHandlePendingSubWayBills");
            $.each(allHandlePendingSubWayBillsBtn, function (i, item) {
                $(item).click(function () {
                    var wbID = $(item).attr("wbID");
                    var wbSerialNum = $(item).attr("wbSerialNum");
                    HanldePendingSubWayBill(wbID, wbSerialNum);
                });
    ...
    function HanldePendingSubWayBill(wbID, wbSerialNum) {
        var div_DetailDlg = self.parent.$("#dlg_GlobalDetail");
        DetailDlg = div_DetailDlg.dialog({
            buttons: [{
                text: '关 闭',
            ...
            title: '待 处 理 货 单 明 细',
            href: HanldePendingSubWayBillURL + wbSerialNum + "&Detail_wbID=" + encodeURI(wbID),
            modal: true,
            resizable: true,
            cache: false,
            left: 0,
